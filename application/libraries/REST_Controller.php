...
abstract class REST_Controller extends MX_Controller
{
    ...
    */
	...
	// Skip keys test for $config['auth_override_class_method']['class'['method'] = 'none'
        ...
        if($this->config->item('rest_auth_required') && in_array( strtolower($this->uri->segment(1)), $this->config->item('rest_auth_required'))){
			...
			if($authToken != "")
			{
				$userAuthData =  !empty($authToken) ? $this->common_model->isValidUser($authToken) : '';
				
				if(!empty($userAuthData))
				{
					if($userAuthData->is_verified == 0)
					{
						$responseArray = array('status'=>0,'message'=>"Sorry, Your account is not verified. Try again later.",'Authorization'=>'');
					...
					else if($userAuthData->is_active == 0)
					{
						$responseArray = array('status'=>0,'message'=>"Sorry, Your account is not active kindly please contact admin.",'Authorization'=>'');
						$response = $this->generate_response($responseArray);
						$this->response($response,OK);
					...
					else if($userAuthData->is_deleted == 1)
					{
						$responseArray = array('status'=>0,'message'=>"Sorry, We did not found any information with provided token.",'Authorization'=>'');
						$response = $this->generate_response($responseArray);
						$this->response($response,OK);
					...
					$responseArray = array('status'=>0,'message'=>ResponseMessages::getStatusCodeMessage(101),'Authorization'=>'');
					$response = $this->generate_response($responseArray);
					$this->response($response,OK);
			...
			{
				$responseArray = array('status'=>0,'message'=>ResponseMessages::getStatusCodeMessage(123),'Authorization'=>'');
				$response = $this->generate_response($responseArray);
				$this->response($response,OK);
...

    {
        ...
        if (config_item('rest_enable_logging') && !empty($this->_insert_id)) {
...

    {
        ...
        if (config_item('rest_enable_keys') and $use_key and $this->_allow === false) {
            ...
            $this->response(array(config_item('rest_status_field_name') => 100, config_item('rest_message_field_name') => 'Invalid API Key '.$this->rest->key), 403);
...

    {
		...
		if (config_item('rest_enable_logging') && $this->resPrint)
...

    {
		...
		$uID= !empty($this->authData) ? $this->authData->ID : '0';
